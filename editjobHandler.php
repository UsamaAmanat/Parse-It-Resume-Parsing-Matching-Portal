...
if ($Recruiter) {
    ...
    $queryforInsert = "UPDATE jobs SET jobs_clientName='$name',jobs_recruterName='$Recruiter',jobs_title='$Title',jobs_matchingKeywords='$Keywords',fname='$fname',lname='$lname',email='$Email',phone='$Phone',addresss='$Addresss',jobtype='$jobtypedd',cname='$cname' WHERE jobs_id='$id' ";

    // $queryforInsert = "U INTO jobs(jobs_clientName, jobs_recruterName, jobs_title, jobs_status, jobs_matchingKeywords,jobs_date, jobs_totalResume, jobs_cid,   fname, lname, email, phone, addresss, jobtype, cname) VALUES ('$name', '$Recruiter', '$Title', 'Lodged',  '$Keywords', '$cdatee', 'none', '$ClientId', '$fname', '$lname', '$Email', '$Phone', '$Addresss', '$jobtypedd','$cname') ";

    if (mysqli_query($conn, $queryforInsert)) {
        echo json_encode(array("statusCode" => 200, "lastid" => $id));
    } else {
